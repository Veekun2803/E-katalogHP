...
<head>
    ...
    <style>
        ...
        .thumbnail-active { border-color: #2563eb !important; ring: 2px; ring-offset: 2px; --tw-ring-color: #2563eb; }
...
<div class="max-w-6xl mx-auto p-4 md:p-8">
    ...
    <nav class="flex mb-6 text-sm font-medium text-gray-500">
        ...
        <span class="text-gray-800 line-clamp-1 font-bold"><?= htmlspecialchars($data['nama_hp']) ?></span>
    </nav>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div class="grid md:grid-cols-2 gap-0">
            ...
            <div class="p-6 md:p-10 bg-gray-50/50 border-r border-gray-100">
                <div class="sticky top-10">
                    <div class="w-full aspect-square bg-white rounded-[2rem] shadow-inner mb-6 flex items-center justify-center overflow-hidden border border-gray-100 p-8">
                        <img id="mainImage" 
                             src="<?= $gambarList[0] ?>" 
                             class="max-h-full max-w-full object-contain transition-all duration-500 transform hover:scale-105">
                    </div>

                    <div class="flex gap-3 justify-center flex-wrap">
                        <?php foreach ($gambarList as $index => $g): ?>
                            <div onclick="changeImage('<?= $g ?>', this)" 
                                 class="thumbnail-item w-16 h-16 md:w-20 md:h-20 bg-white rounded-2xl overflow-hidden border-2 border-transparent shadow-sm cursor-pointer flex items-center justify-center transition-all hover:border-blue-300 <?= $index === 0 ? 'thumbnail-active' : '' ?>">
            ...
            <div class="p-6 md:p-12">
                <div class="mb-2">
                    <span class="bg-blue-100 text-blue-700 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest italic">
                        <?= htmlspecialchars($data['brand']) ?>
                    </span>
                </div>

                <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 mb-4 leading-tight tracking-tight">
                ...
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <?php else: ?>
                        <div class="text-red-500 bg-red-50 px-3 py-1 rounded-lg text-sm font-bold uppercase tracking-tighter">Habis</div>
                    <?php endif; ?>
                </div>

                <!-- Input Qty & Tombol -->
                <div class="flex flex-col sm:flex-row gap-4 mb-10 pb-10 border-b border-gray-100 items-center">
                    <div class="flex items-center bg-gray-100 rounded-2xl p-1 shadow-inner">
                        ...
                        <input type="number" id="qtyInput" value="1" min="1" max="<?= $data['stok'] ?>" readonly
                    ...
                    <button type="button" 
                            ...
                            id="btnCart"
                            class="w-full sm:w-auto px-10 bg-blue-600 text-white font-black py-4 rounded-2xl hover:bg-blue-700 transition shadow-lg shadow-blue-100 active:scale-95 disabled:bg-gray-300 disabled:shadow-none flex items-center justify-center gap-3 uppercase tracking-widest text-xs">
                        <svg id="iconCart" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="space-y-6 text-gray-700">
                    <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                        <h3 class="font-black text-gray-900 mb-3 flex items-center gap-2 uppercase text-xs tracking-widest italic">
                        ...
                        <p class="text-sm leading-relaxed text-gray-600 font-medium">
                            <?= nl2br(htmlspecialchars($data['deskripsi'])) ?>
                        </p>
                    </div>

                    <div class="p-6 border border-gray-100 rounded-3xl">
                        <h3 class="font-black text-gray-900 mb-4 flex items-center gap-2 uppercase text-xs tracking-widest italic">
                        ...
                        <div class="text-sm leading-relaxed text-gray-600 grid grid-cols-1 gap-2">
                            ...
                            foreach($specs as $spec):
                                if(trim($spec) != ""): ?>
                                    <div class='flex border-b border-gray-50 py-2 group'>
                                        <span class='w-1/3 font-bold text-gray-400 uppercase text-[9px] tracking-widest'>• Detail</span> 
                                        <span class='w-2/3 font-semibold group-hover:text-blue-600 transition-colors'><?= htmlspecialchars($spec) ?></span>
                                    </div>
                            <?php 
                                endif; 
                            endforeach; ?>
    ...
    .then(data => {
        if(data.status === 'success' || data.success === true) {
            Swal.fire({
                icon: 'success',
                title: 'Mantap!',
                text: qty + ' unit berhasil masuk keranjang.',
            ...
            btnSpan.innerText = 'Berhasil Ditambahkan!';
        } else {
